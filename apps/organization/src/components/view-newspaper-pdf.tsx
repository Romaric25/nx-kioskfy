...
import { useSelectedNewspaper } from "@/lib/selected-newspaper-store";
...
export const NewspaperPdfViewer = ({ scale = 0.6, back }: Props) => {
  ...
  const { newspaperId: selectedNewspaperId } = useSelectedNewspaper();

  const {
    data: newspaper,
    isLoading: newspaperLoading,
    isError: newspaperError,
  } = useQuery({
    queryKey: ["labo-newspaper", selectedNewspaperId],
    queryFn: () => api.newspapers.getById(selectedNewspaperId),
    enabled: !!selectedNewspaperId,
  });
  ...
  // No newspaper selected (direct URL access)
  if (!selectedNewspaperId) {
    return (
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="text-center space-y-4">
          <FileX2 className="mx-auto h-12 w-12 text-muted-foreground" />
          <h3 className="font-semibold text-lg">Aucun journal sélectionné</h3>
          <p className="text-muted-foreground text-sm">
            Choisissez un journal depuis la liste pour le visualiser.
          </p>
          <Link to={back}>
